...
export default function ImmunitySupportPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ FILTER ONLY IMMUNITY SUPPORT PRODUCTS FROM WOOCOMMERCE
        const immunityProducts = allProducts
          .filter((p) =>
            p.categories?.some(
              (cat) => cat.slug === "immunity-support"
            )
          )
          .map((p) => ({
            id: p.id,
        ...
        setProducts(immunityProducts);
        setFiltered(immunityProducts);
      } catch (error) {
        console.error("Failed to load Immunity Support products", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="products-area">
          <div className="products-grid">
            {filtered.length > 0 ? (
              ...
              <p>No Immunity Support products found</p>
            )}
          </div>
        </div>
      </div>
