  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ FILTER ONLY MULTIVITAMINS PRODUCTS FROM WOOCOMMERCE
        const multivitaminProducts = allProducts
          .filter((p) =>
            p.categories?.some(
              (cat) => cat.slug === "multivitamins"
            )
          )
          .map((p) => ({
            id: p.id,
        ...
        setProducts(multivitaminProducts);
        setFiltered(multivitaminProducts);
      } catch (error) {
        console.error("Failed to load Multivitamin products", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="products-area">
          <div className="products-grid">
            {filtered.length > 0 ? (
              ...
              <p>No Multivitamin products found</p>
            )}
          </div>
        </div>
      </div>
