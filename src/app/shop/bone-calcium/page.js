...
export default function BoneCalciumPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ FILTER ONLY BONE & CALCIUM PRODUCTS FROM WOOCOMMERCE
        const boneCalciumProducts = allProducts
          .filter((p) =>
            p.categories?.some(
              (cat) => cat.slug === "bone-calcium"
            )
          )
          .map((p) => ({
            id: p.id,
        ...
        setProducts(boneCalciumProducts);
        setFiltered(boneCalciumProducts);
      } catch (error) {
        console.error("Failed to load Bone & Calcium products", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="products-area">
          <div className="products-grid">
            {filtered.length > 0 ? (
              ...
              <p>No Bone & Calcium products found</p>
            )}
          </div>
        </div>
      </div>
