...
export default function JointCarePage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ FILTER ONLY JOINT CARE PRODUCTS FROM WOOCOMMERCE
        const jointCareProducts = allProducts
          .filter((p) =>
            p.categories?.some(
              (cat) => cat.slug === "joint-care"
            )
          )
          .map((p) => ({
            id: p.id,
        ...
        setProducts(jointCareProducts);
        setFiltered(jointCareProducts);
      } catch (error) {
        console.error("Failed to load Joint Care products", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="products-area">
          <div className="products-grid">
            {filtered.length > 0 ? (
              ...
              <p>No Joint Care products found</p>
            )}
          </div>
        </div>
      </div>
